...
public:
	...
	int PImpInterval = 500;       	// Интервал между импульсами при прокачке

	// Tank Level / Уровень в бачке
	int impPer1CC = 50;	// Колличество импульсов на 1мл (Замерить прокачкой в шприц)
	int tankVolueCC = 200;			// Емкость бачка масла, мл или куб.см.
	float tankActualLevel = -1;	// Фактический остаток в бачке. Пересчитывается каждые 10-20 импульсов.

	// GPS
	int gpsMinCalcSpeed = 3;// Минимальная скорость расчета дистанции движения по GPS, км/ч
	uint32_t gpsAgeMax = 30000;	// Значение максимальной величины "старости" данных, т.е. определяется когда считать данные протухшими, мс.
	uint32_t gpsUpdatePeriod = 1500; // Значение периодичности обновления данных GPS, мс
	uint32_t gpsDataWait = 100;	// Длительность ожидания данных на порту обмена. Наверное должна быть не меньше частоты обновления данных в модуле (в настройках модуля, u-center)
